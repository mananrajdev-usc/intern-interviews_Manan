...
function changeDialValue (index, incrementBy) {
	...
	if (lockState.wheels[index]<0) {
		lockState.wheels[index]=lockState.wheels[index]+10
		
	}
	a=JSON.stringify(JSON.stringify(lockState.wheels))
	b=JSON.stringify(JSON.stringify(SECRET_COMBO))
	if (a==b) {
		lockState.locked = false
		redirect('manan-rajdev')
		console.log("wdwwdw")
	}
